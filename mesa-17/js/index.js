...
function renderizarDatosUsuario() {
    ...
    .then(datos => {
        ...
        tarjeta.innerHTML = `<h3>${datos.results[0].name.first} ${datos.results[0].name.last}</h3></br><h3>${datos.results[0].email}</h3>`;
        let img = document.createElement('img');
        img.setAttribute('src', `${datos.results[0].picture.large}`);
